...
folders <- tibble(name = names(sizes), folder_size = unlist(sizes))

# Total disk space consumed
space_consumed <- folders %>% filter(name == "base") %>% .$folder_size
space_available <- max_space - space_consumed
additional_space_required <- space_req - space_available

#Mark folders
folders %>% 
  mutate(should_be_deleted = case_when(
    folder_size >= additional_space_required ~ TRUE, TRUE ~ FALSE
  )) %>% 
  filter(should_be_deleted) %>%
  filter(folder_size == min(folder_size))